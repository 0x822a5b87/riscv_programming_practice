...
extern unsigned long asm_bl_test(unsigned long a, unsigned long b);

void bl_test() {
        const unsigned long v1 = asm_bl_test(0, 9);
        if (v1 == 11)
                uart_send_string("bl test ok\n");
        else
                uart_send_string("bl test case 1 failed\n");

        const unsigned long val2 = asm_bl_test(5, 2);
        if (val2 == 1)
                uart_send_string("bl test ok\n");
        else
                uart_send_string("bl test case 2 failed\n");
}
