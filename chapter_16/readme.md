...
1. CPU内部存在无效队列和存储缓冲区，用于提高CPU的性能。
2. 从硬件的层面讲：
   1. 读屏障作用于无效队列，读屏障（如 smp_rmb()）会强制 “清理无效队列中已收到的无效信号”，确保读屏障后的 “读取操作” 能拿到最新数据；
   2. 写屏障作用于存储缓冲区，保证所有写屏障后的指令存储缓冲区刷新后才执行；
3. 从软件的层面讲：
   1. 读屏障保证所有读屏障后的指令，在读屏障之前的指令加载完成之后才执行；
   2. 写屏障保证所有写屏障后的指令，在写屏障之前的指令存储完成之后才执行；
4. 程序包括程序顺序和内存顺序，程序顺序是程序写好的时候就确定的，而内存顺序则是指令的实际执行顺序，他是不确定的。和硬件架构等因素有关。程序顺序和内存顺序不一致，也是导致内存一致性问题的原因。

>为什么读屏障不需要作用于存储缓冲区？

这个问题的核心在于 读屏障（`smp_rmb()`）的设计目标是 “解决读操作的旧值问题”，而存储缓冲区的作用是 “暂存写操作”—— 两者负责的硬件组件和问题场景完全不同，因此读屏障无需处理存储缓冲区。

先明确两个关键前提：

1. 存储缓冲区的作用范围：仅用于暂存当前 CPU 的 “写操作”（如a=1），目的是避免 CPU 等待总线响应而停滞。它只影响 “写操作何时对其他 CPU 可见”，不影响 “当前 CPU 的读操作”。
2. 读屏障的设计目标：解决 “当前 CPU 读取共享数据时，可能因无效队列未清理而拿到旧缓存” 的问题，核心针对 “读操作的时效性”，不涉及 “写操作的暂存”。
...
1. `while (b == 0) continue` 语句保证了，当跳出循环的时候 `b = 1` 已经执行；
2. `smp_wmb()` 保证了，在 `b = 1` 执行的时候，`a = 1` 一定已经执行完毕并且将存储缓冲区刷新。
3. `smp_rmb()` 保证了，执行 `a == 1` 之前，`b = 1` 的 BusRdX 信号已经从无效队列刷新。

这个例子中，如果没有读写屏障，那么可能会有几个地方触发读取异常：

1. 没有写屏障，那么 `b = 1` 执行完毕的时候，`a = 1` 可能还在存储缓冲区，这样会导致 `a == 1` 判定失败；
2. 没有读屏障，那么 `a == 1` 执行的时候，`a = 1` 触发的BusRdX可能还在无效队列；
